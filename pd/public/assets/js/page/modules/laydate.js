...
win.laydate = function(options){
    options = options || {};
     _date = options.date || window._date ; 
     console.log(_date);
    try{
...
Dates.block_td =function(){
      var S = Dates.query; 
    var trs =   S("#laydate_table tr");
    // console.log( trs );
    // alert(document.documentElement.clientHeight);
    var clientHeight = document.documentElement.clientHeight ; 
   for(var i = 1 ; i < trs.length ; i++){
      trs[i].style.height = ( 500*0.9/7 ).toFixed(2)  + "px";
   }

}

Dates.date_in_arr= function(val , arr){
    arr = arr || [] ;
   var  l = arr.length ; 
    if(arr.length == 0 ) return false ; 
  for( var i = 0 ; i<l ; i++){
    if(val == arr[i].sign_date){
        return true ; 
    } 
  }
  return false ; 
...
Date.act_tag =function(date){
   ...
   if( dateInfo != undefined ) {
     ...
     var arr ;
     if(dateInfo.remain_guest >=20){

         arr=  [
          "<span remain='",dateInfo.remain_guest,"' class='date-remain'>", "余位充足","</span>",  
          "<span tourCode='",dateInfo.tourCode,"' class='tour-code'> </span>"
      ];

     }else{
         arr=  [
          "<span remain='",dateInfo.remain_guest,"' class='date-remain'>余",dateInfo.remain_guest ,"</span>",  
          "<span tourCode='",dateInfo.tourCode,"' class='tour-code'> </span>"
       ];
     }
     return  arr.join("");
...
Dates.viewDate = function(Y, M, D){
    ...
    Dates.each(as.tds, function(i, elem){
        var YY = log.ymd[0], MM = log.ymd[1] + 1, DD ,is_disabled;
        elem.className = '';
        ...
        if(i < log.FDay){
            ...
            MM === 1 && (YY -= 1);
            MM = MM === 1 ? 12 : MM - 1; 
            // console.log( Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY );
            // console.log( _date );
           is_disabled =  (_date[Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY ] == undefined) ; 
            
            if( is_disabled ){
              elem.innerHTML = "<span class='date-num'>"+ DD+"</span>"  ;
              Dates.addClass(elem, 'td-disabled');
            }else{
              elem.innerHTML = "<span class='date-num'>"+ DD+"</span>" + Date.act_tag( Dates.digit(DD)+ '/'+Dates.digit(MM)+'/'+YY ) ;
              Dates.addClass(elem, 'laydate_nothis passed td-abled');
           ...
           is_disabled =  (_date[Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY ] == undefined) ; 
            ...
            if( is_disabled ){
              ...
              elem.innerHTML = "<span class='date-num'>"+ DD+"</span>" + Date.act_tag( _date[Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY ])   ;
              Dates.addClass(elem, ' td-abled');
           ...
           is_disabled =  (_date[Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY ] == undefined) ; 
            ...
            if( is_disabled ){
              ...
              Dates.addClass(elem, 'td-disabled');
               ...
               elem.innerHTML =  "<span class='date-num'>"+ DD+"</span>" +  Date.act_tag(_date[Dates.digit(DD)+ '/'+ Dates.digit(MM)+'/'+YY] )  ;
               Dates.addClass(elem, 'laydate_nothis future td-abled');
            }
        ...
        if( YY ==now.getFullYear() && MM == now.getMonth()+1 && DD == now.getDate()){
              Dates.addClass(elem, ' today');
        }

        // if( ( new Date(YY , MM-1 , DD) ).getDay() == 0 || (new Date(YY , MM-1 , DD)).getDay() == 6 ){ //周末
        //     Dates.addClass(elem, 'weekend');
        // }

        //  if(Dates.date_in_arr(YY+'.'+Dates.digit(MM)+'.'+ Dates.digit(DD), datas.userInfo.signList )  ){ //签到
        //     console.log( YY+'.'+Dates.digit(MM)+'.'+ Dates.digit(DD) , datas.userInfo.signList   );
        //     Dates.addClass(elem, ' singed');
        // }
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        div = doc[creat]('div');
        div.id = as[0];
        div.className = as[0] +" animated fadeIn";
        ...
        div.setAttribute('name', 'laydate-v'+ laydate.v);
          ...
          +'<div class="laydate_ym laydate_m" id="laydate_MM">'
            +'<a    class="laydate_choose left laydate_chprev laydate_tab"><cite></cite></a>'
            +'<div style=" width:200px; margin: 0 auto   ">'
            +'<input id="laydate_y" class="year" readonly>'
            +'<input id="laydate_m"  class="month"  readonly>'
            +'</div>'
            // +'<label></label>'
        ...
        + Dates.viewtb
        +'<div  style="display:none" class="laydate_bottom" style="display:none">'
          +'<ul id="laydate_hms">'
        ...
        // console.log(options, S(options.contains) );
        // S(options.contains).appendChild(div); 
        // options.contains(div);
        Dates.box = S('#'+as[0]);        
        Dates.events();
        div = null;
    } else {
...
Dates.events = function(){
    ...
    //切换月
    Dates.tabMonth = log.tabMonth = function(type){
        if(type){
            Dates.ymd[1]++;
            if(Dates.ymd[1] === 12){
                Dates.ymd[0]++;
    ...
    Dates.each(S('#laydate_MM .laydate_tab'), function(i, elem){
        Dates.on(elem, "click", function(ev){
            console.log("#laydate_MM .laydate_tab");
            Dates.stopmp(ev).reshow();
            log.tabMonth(i);
        });
    });
    
     var aa =   S('#laydate_MM .laydate_tab');
  console.log(aa);
