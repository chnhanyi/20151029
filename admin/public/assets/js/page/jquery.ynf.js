(function($, win) {
	...
	ynf.init_skin = function(){ //初始化皮肤
        if(window.location.origin.indexOf("mobile.hmeili.com") == -1 ){ //测试环境
           $(document.body).addClass('skin-2');
           document.title=  document.title+ "--测试环境" ;
           $(".nav-user-photo").attr("src", "../assets/avatars/user2.png");
	...
	ynf.get_time_rules = function(args) { // 得到时间过滤规则
		...
		var begin_date = $begin.find(".date-picker").val() ;
		end_date = $end.find(".date-picker").val() ;
		if (begin_date == ""   || end_date == "" ) {
			return "error";
		}
		// var begin_time_num =   begin_date.replace(/[\-\s]/g, "") ;
		// var end_time_num =  end_date.replace(/[\-\s]/g, "")  ;
